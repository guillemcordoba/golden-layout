...
export abstract class LayoutManager extends EventEmitter {
  ...
  createDragProxy:
    | ((item: ComponentItem, x: number, y: number) => void)
    | undefined;
  ...
  ): ComponentContainer.Handle;
  /** @internal */
  abstract unbindComponent(
    container: ComponentContainer,
    handle: ComponentContainer.Handle
  ): void;
  ...
  private onDrag(event: MouseEvent) {
    const x = event.pageX;
    const y = event.pageY;
    if (this._itemAreas === null) this.calculateItemAreas();
    if (this._itemAreas === null || this._itemAreas.length === 0) return;
  ...
  removeElementEventually(element: HTMLElement): void {
    if (this.currentlyDragging()) {
      ...
      this._actionsOnDragEnd.push((cancel) => {
        if (cancel) element.style.opacity = "";
        else element.remove();
      });
    } else {
      element.remove();
  ...
  deferIfDragging(action: (cancel: boolean) => void): void {
    if (this.currentlyDragging()) this._actionsOnDragEnd.push(action);
    else action(false);
  }
  ...
  startComponentDrag(ev: DragEvent, componentItem: ComponentItem): void {
    ...
    if (useFreshDragImage) {
      ...
      inner.style.height = `${
        stackBounds.height - tabsContainer.clientHeight
      }px`;
    ...
    if (!isActiveTab) {
      for (const sibling of stack.contentItems) {
        if (sibling !== componentItem) sibling.element.style.opacity = "0";
      }
    }
    ...
    tabsContainer.style.marginLeft = `${
    ...
    this._removeItem = () => {
      ...
      if (
        this.layoutConfig.settings.showOldPositionWhenDragging &&
        stack.parent &&
      ...
      if (!isActiveTab) {
        for (const sibling of stack.contentItems) {
          if (sibling !== componentItem) sibling.element.style.opacity = "";
        }
      }
      tabElement.style.visibility = "";
      headerElement.style.visibility = "";
      if (useFreshDragImage) image.remove();
      else image.style.opacity = oldOpacity;
  ...
  private onDragEnter(e: MouseEvent) {
    if (this._removeItem) this._removeItem();
    e.stopPropagation();
    if (e instanceof DragEvent) {
      if (e.dataTransfer) e.dataTransfer.dropEffect = "move";
      if (!this.validDragEvent(e)) return;
    }
  ...
  private onDragOver(e: MouseEvent) {
    ...
    this.onDrag(e);
    if (valid && this._area) e.preventDefault(); // allow drop
  }
  ...
  private onDragEnd(
    ...
    console.log(
      "onDragEnd st:" + this._dragState + " timer:" + this.delayedDragEndTimer
    );
    ...
    if (
      this._draggedComponentItem &&
      this._dragState == DragState.DroppedInThisWindow
    ) {
      if (onlyWindow) cancel = true;
      else {
    ...
    if (
      ...
      if (parent && parent.type === "stack") parent.element.style.display = "";
  ...
  public droppedInOtherWindow(): void {
    this._dragState = DragState.DroppedElsewhere;
    if (this.delayedDragEndFunction) this.delayedDragEndFunction();
  }
  ...
  private onDrop(e: MouseEvent) {
    ...
    if (this._area !== null) {
      if (this._draggedComponentItem) {
        ...
        console.log(
          "dropped from different window " + JSON.stringify(data.config)
        );
        const item = new ComponentItem(
          this,
          data.config,
          this.groundItem as ComponentParentableItem
        );
